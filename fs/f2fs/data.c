...
static void __read_end_io(struct bio *bio, bool compr)
{
	struct page *page;
	struct bio_vec *bv;
	int i;
	...
	bio_for_each_segment_all(bv, bio, i) {
		...
		if (compr && f2fs_is_compressed_page(page)) {
			f2fs_decompress_pages(bio, page);
			continue;
		}
#endif
...
static void __f2fs_read_end_io(struct bio *bio, bool compr)
{
	if (!compr)
		__read_end_io(bio, false);
	f2fs_release_read_bio(bio);
}

static void f2fs_decompress_bio(struct bio *bio)
{
	__read_end_io(bio, true);
}
...
static void bio_post_read_processing(struct bio_post_read_ctx *ctx)
{
	...
	__f2fs_read_end_io(ctx->bio, false);
}
...
static struct bio *f2fs_grab_read_bio(struct inode *inode, block_t blkaddr,
				      ...
				      bool for_write, bool for_verity)
{
	struct f2fs_sb_info *sbi = F2FS_I_SB(inode);
	struct bio *bio;
	struct bio_post_read_ctx *ctx;
				 ...
				 block_t blkaddr, int op_flags, bool for_write)
{
	...
	bio = f2fs_grab_read_bio(inode, blkaddr, 1, op_flags, for_write, true);
	if (IS_ERR(bio))
		return PTR_ERR(bio);
...
submit_and_realloc:
	...
	if (bio == NULL) {
		bio = f2fs_grab_read_bio(inode, block_nr, nr_pages,
				is_readahead ? REQ_RAHEAD : 0, false, true);
		if (IS_ERR(bio)) {
			ret = PTR_ERR(bio);
			bio = NULL;
			goto out;
...
out_put_dnode:
	f2fs_put_dnode(&dn);
out:
	f2fs_decompress_end_io(cc->rpages, cc->cluster_size, true);
	*bio_ret = bio;
	return ret;
}
#endif
